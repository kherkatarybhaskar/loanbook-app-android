...
import { View, Text, StyleSheet, FlatList, Image, Dimensions, TouchableOpacity, Modal, TextInput, Button } from 'react-native';
...
const Loans = () => {
  ...
  const fetchBorrowers = async () => {
    ...
    try {
      const response = await fetch(`${ip}/api/loan`); // Use /depositors route
      // const response = await fetch(`http://${ip}:5000/api/loan`); // Use /depositors route
    ...
    const fetchfilteredBorrowers = async (filters) => {
        ...
        try {
            const response = await fetch(`${ip}/api/loan/account/filtered?${params.toString()}`);
            // const response = await fetch(`http://${ip}:5000/api/loan/account/filtered?${params.toString()}`);
            const data = await response.json();
    ...
    const handleFilterSubmit = () => {
        ...
        fetchfilteredBorrowers(filters);
  ...
  useFocusEffect(
    useCallback(() => {
      fetchBorrowers(); // Re-fetch depositors whenever the screen is focused
    }, [])
  );
  ...
  const renderCustomerCard = ({ item }) => {
    return (
      <TouchableOpacity 
        ...
        onPress={() => {
          // Navigate to Borrow.js and pass customerName and accountNumber as parameters
